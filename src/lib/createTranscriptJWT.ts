...
/**
 * Creates a new JWT that can be used to access the transcript with the given uid
 * for the specified duration in seconds (or 30 minutes if not specified).
 */
export const createTranscriptJWT = async (
  uid: string,
  opts?: { durationSeconds?: number }
): Promise<string> => {
  const duration = opts?.durationSeconds ?? DEFAULT_DURATION;
  const secret = process.env.OSEH_TRANSCRIPT_JWT_SECRET;
